...
func (bfr *BoundedFrequencyRunner) Loop(stop <-chan struct{}) {
	klog.V(3).InfoS("BoundedFrequencyRunner Loop running", "runner", bfr.name)
	bfr.timer.Reset(bfr.maxInterval)
	for {
		select {
		case <-stop:
			bfr.stop()
			klog.V(3).InfoS("BoundedFrequencyRunner Loop stopping", "runner", bfr.name)
			return
		case <-bfr.timer.C():
			bfr.tryRun()
		case <-bfr.run:
...
func (bfr *BoundedFrequencyRunner) doRetry() {
	...
	if retryInterval < bfr.timer.Remaining() {
		klog.V(3).InfoS("retrying", "BoundedFrequencyRunner", bfr.name, "retryInterval", retryInterval)
		bfr.timer.Stop()
...
func (bfr *BoundedFrequencyRunner) tryRun() {
	...
	if bfr.limiter.TryAccept() {
		...
		bfr.timer.Stop()
		bfr.timer.Reset(bfr.maxInterval)
		klog.V(3).InfoS("ran", "BoundedFrequencyRunner", bfr.name, "minInterval", bfr.minInterval, "maxInterval", bfr.maxInterval)
		return
	}
